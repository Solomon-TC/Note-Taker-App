...
const Toolbar = ({ editor, onInsertImage, onInsertDrawing }: ToolbarProps) => {
  ...
  const insertTable = useCallback(() => {
    ...
    try {
      ...
      if ((editor as any).can().insertTable(tableConfig)) {
        const result = (editor as any).chain().focus().insertTable(tableConfig).run();
  ...
  const addTableRow = useCallback(() => {
    ...
    const result = (editor as any).chain().focus().addRowAfter().run();
  ...
  const addTableColumn = useCallback(() => {
    ...
    const result = (editor as any).chain().focus().addColumnAfter().run();
  ...
  const deleteTableRow = useCallback(() => {
    ...
    const result = (editor as any).chain().focus().deleteRow().run();
  ...
  const deleteTableColumn = useCallback(() => {
    ...
    const result = (editor as any).chain().focus().deleteColumn().run();
  ...
  const deleteTable = useCallback(() => {
    ...
    const result = (editor as any).chain().focus().deleteTable().run();
    console.log("Delete table result:", result);
  }, [editor]);

  // Enhanced table state detection with debugging
  const isInTable = editor?.isActive("table") ?? false;
  const canAddRow = (editor as any)?.can().addRowAfter() ?? false;
  const canAddColumn = (editor as any)?.can().addColumnAfter() ?? false;
  const canDeleteRow = (editor as any)?.can().deleteRow() ?? false;
  const canDeleteColumn = (editor as any)?.can().deleteColumn() ?? false;
  const canDeleteTable = (editor as any)?.can().deleteTable() ?? false;
    ...
    (hex: string) => {
      ...
      try {
        ...
        console.log("Editor state before color:", {
          canSetColor: (editor as any).can().setColor
            ? (editor as any).can().setColor(hex)
            : false,
        ...
        try {
          result = (editor as any).chain().focus().setColor(hex).run();
        ...
        if (!result) {
          ...
          if (!result) {
            try {
              result = (editor.commands as any).setColor(hex);
  ...
  const clearTextColor = useCallback(() => {
    ...
    try {
      ...
      console.log("Editor state before clear:", {
        canUnsetColor: (editor as any).can().unsetColor
          ? (editor as any).can().unsetColor()
          : false,
        currentColor: editor.getAttributes("textStyle").color,
      });

      // CRITICAL FIX: Use unsetColor command from Color extension
      const result = (editor as any).chain().focus().unsetColor().run();
    ...
    (hex: string) => {
      ...
      try {
        ...
        console.log("Editor state before highlight:", {
          canSetHighlight: (editor as any).can().setHighlight
            ? (editor as any).can().setHighlight({ color: hex })
            : false,
          hasSelection: from !== to,
          selectionLength: to - from,
          isFocused: editor.isFocused,
        ...
        try {
          result = (editor as any).chain().focus().setHighlight({ color: hex }).run();
        ...
        if (!result) {
          ...
          if (!result) {
            try {
              result = (editor.commands as any).setHighlight({ color: hex });
  ...
  const clearHighlight = useCallback(() => {
    ...
    try {
      ...
      console.log("Editor state before clear:", {
        canUnsetHighlight: (editor as any).can().unsetHighlight(),
        isHighlightActive: editor.isActive("highlight"),
        currentHighlight: editor.getAttributes("highlight"),
      });

      const result = (editor as any).chain().focus().unsetHighlight().run();
